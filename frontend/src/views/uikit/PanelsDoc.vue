...
const emit = defineEmits(['updateActiveEmployeeCount']);
...
const fetchTaches = async () => {
  try {
    ...
    if (response.data?.data?.employees?.employees) {
      ...
      // Emit the activeEmployeeCount after fetching the data
      emit('updateActiveEmployeeCount', activeEmployeeCount.value);
    } else {
...
// Computed property to calculate the number of active employees
const activeEmployeeCount = computed(() => {
  return taches.value.filter(emp => !emp.disabledUntil).length;
});
        ...
        <Column headerStyle="width: 3rem">
          <template #body="{ data }">
            <div class="flex align-items-center justify-content-center">
              <i
                class="pi"
                :class="{'pi-circle-fill text-green-700': !data.disabledUntil, 'pi-circle-fill text-red-500': data.disabledUntil}"
